...
export function generateReceiptPdf(r: Record<string, unknown>) {
  ...
  for (const [index, item] of (data.itemsWithGst as Array<Record<string, unknown>>).entries()) {
    ...
    const qty = isWeighted
      ? `${Number(item.quantity || 0).toFixed(3)}kg`
    ...
    doc.text(wrappedName[0] || "", x1, y);
    doc.text(qty, x2 + c2, y, { align: "right" });
    doc.text(rate, x3 + c3, y, { align: "right" });
    doc.text(amt, x4 + c4, y, { align: "right" });
    y += LH + 1;
  ...
  if (s.discount && (s.discount as number) > 0) {
    const label =
      s.discountPercent && (s.discountPercent as number) > 0
  ...
  y += SM;
  txt(`Payment Mode : ${s.paymentMode}${s.paymentRef ? `  Ref No: ${s.paymentRef}` : ""}`, {
    size: FONT_SIZE - 1,
  });

  // ── GST Breakup ──
  const gstByRate = s.gstByRate as
    | Record<string, { taxable: number; cgst: number; sgst: number }>
    | undefined;
  if (gstByRate && Object.keys(gstByRate).length > 0) {
    ...
    for (const slab of slabs) {
      txt(
        pad(`${slab.rate}%`, gc1) +
          pad(fmt(slab.taxable), gc2, "r") +
          pad(fmt(slab.cgst), gc3, "r") +
          pad(fmt(slab.sgst), gc4, "r") +
    ...
    txt(
      pad("Total", gc1) +
        pad(fmt(s.taxableValue as number), gc2, "r") +
        pad(fmt(s.totalCgst as number), gc3, "r") +
        pad(fmt(s.totalSgst as number), gc4, "r") +
  ...
  if (data.storePhone)
    txt(`Customer Care : ${data.storePhone}`, { size: 7, align: "center", spacing: SM });
