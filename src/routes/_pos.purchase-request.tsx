...
import {
  ArrowLeft, AlertTriangle, TrendingUp, Clock, Wallet,
  Sparkles, Plus, Minus, Save, Send, FileText, Building2, CheckSquare,
} from "lucide-react";

export const Route = createFileRoute("/_pos/purchase-request")({
  head: () => ({ meta: [{ title: "Purchase Request — CHOTA BAZAAR" }] }),
...
const ROWS: PR[] = [
  { id: "pr1", emoji: "🥛", name: "Amul Gold Milk 1L", brand: "Amul", current: 18, demand: 240, qty: 240, supplier: "Amul Federation", cost: 56, alert: "Stock runs out in 2 days" },
  { id: "pr2", emoji: "🍞", name: "Britannia Bread 400g", brand: "Britannia", current: 8, demand: 120, qty: 144, supplier: "Britannia Distributors", cost: 32, alert: "Demand ↑ 28% this week" },
  { id: "pr3", emoji: "🍚", name: "Daawat Basmati Rice 5kg", brand: "Daawat", current: 4, demand: 60, qty: 80, supplier: "ITC Foods Pvt Ltd", cost: 480, alert: "Critical — reorder now" },
  { id: "pr4", emoji: "🌾", name: "Aashirvaad Atta 10kg", brand: "Aashirvaad", current: 12, demand: 90, qty: 100, supplier: "ITC Foods Pvt Ltd", cost: 420, alert: "Festival demand spike forecast" },
  { id: "pr5", emoji: "🧂", name: "Madhur Sugar 1kg", brand: "Madhur", current: 22, demand: 180, qty: 200, supplier: "ITC Foods Pvt Ltd", cost: 42, alert: "Below safety stock" },
  { id: "pr6", emoji: "🛢️", name: "Fortune Sunflower Oil 5L", brand: "Fortune", current: 6, demand: 48, qty: 60, supplier: "ITC Foods Pvt Ltd", cost: 720, alert: "Stock out in 3 days" },
  { id: "pr7", emoji: "🍪", name: "Parle-G Biscuits 250g", brand: "Parle", current: 32, demand: 360, qty: 400, supplier: "Parle Products", cost: 22, alert: "Fast moving · MOQ optimized" },
  { id: "pr8", emoji: "🥤", name: "Coca-Cola 750ml × 24", brand: "Coca-Cola", current: 14, demand: 96, qty: 120, supplier: "HUL Direct", cost: 720, alert: "Weekend demand +35%" },
  { id: "pr9", emoji: "🧺", name: "Surf Excel Powder 1kg", brand: "Surf Excel", current: 11, demand: 72, qty: 80, supplier: "HUL Direct", cost: 195, alert: "Recommended reorder now" },
  { id: "pr10", emoji: "🧼", name: "Dove Soap 100g × 4", brand: "Dove", current: 18, demand: 120, qty: 144, supplier: "HUL Direct", cost: 240, alert: "Promotion live · stock up" },
];
...
function PurchaseRequestPage() {
  ...
  const setQty = (id: string, q: number) => setRows(r => r.map(x => x.id === id ? { ...x, qty: Math.max(0, q) } : x));
    ...
    <div className="flex h-full flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs font-semibold text-muted-foreground">
              PR-2026-01142 · CB Gurgaon Sector 56 · Requested by Rajesh Khanna · {new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}
            </div>
          </div>
          <span className="rounded-xl bg-[var(--brand-orange)] px-3 py-1.5 text-xs font-extrabold uppercase text-white">Auto-Generated</span>
        </div>

        <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
          <Kpi icon={AlertTriangle} color="var(--brand-red)" label="Critical SKUs" value="14" sub="reorder immediately" />
          <Kpi icon={Clock} color="var(--brand-orange)" label="Projected Stockouts" value="9" sub="within 5 days" />
          <Kpi icon={TrendingUp} color="var(--brand-blue)" label="Avg Supplier Lead" value="3.4d" sub="across vendors" />
          <Kpi icon={Wallet} color="var(--brand-green)" label="Purchase Budget" value={formatINR(budget)} sub={`${formatINR(budget - totalValue)} remaining`} />
        </div>
      </div>
      ...
      <div className="grid flex-1 grid-cols-[1fr_360px] overflow-hidden">
        ...
        <div className="flex flex-col overflow-hidden">
          <div className="flex items-center gap-2 border-b bg-[var(--surface)] px-5 py-2.5">
            <div className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Auto-Generated Procurement List · {rows.length} SKUs</div>
                      ...
                      <td className="px-2 py-2">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[11px] font-semibold text-[var(--brand-orange)]">{r.alert}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-2 py-2 text-right">
                        <span className="text-base font-extrabold tabular-nums" style={{ color: r.current < 10 ? "var(--brand-red)" : "var(--ink)" }}>{r.current}</span>
                      </td>
                      <td className="px-2 py-2 text-right text-sm font-bold tabular-nums">{r.demand}/wk</td>
                      <td className="px-2 py-2">
                        <div className="mx-auto flex w-fit items-center gap-1.5 rounded-2xl border-2 bg-white p-1">
                          <button onClick={() => setQty(r.id, r.qty - 10)} className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--secondary)]">
                            <Minus className="h-4 w-4" />
                          </button>
                          <div className="w-16 text-center text-lg font-extrabold tabular-nums">{r.qty}</div>
                          <button onClick={() => setQty(r.id, r.qty + 10)} className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--brand-blue)] text-white">
                            <Plus className="h-4 w-4" />
                          </button>
                        </div>
                      </td>
                      <td className="px-2 py-2 text-xs font-bold">{r.supplier}</td>
                      <td className="px-2 py-2 text-right text-xs font-semibold text-muted-foreground tabular-nums">{formatINR(r.cost)}</td>
                      <td className="px-2 py-2 text-right text-sm font-extrabold tabular-nums text-[var(--brand-blue)]">{formatINR(r.qty * r.cost)}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <aside className="flex flex-col overflow-y-auto border-l-2 bg-[var(--surface)] p-4">
          ...
          <div className="mt-2 space-y-1.5">
            {[
              ...
              <div key={i} className="flex gap-2 rounded-xl border-2 bg-white p-2.5 text-xs font-semibold">
                <div className="mt-0.5 h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: x.c }} />
                <span>{x.t}</span>
              </div>
            ))}
          </div>
          ...
          <select value={supplier.name} onChange={e => setSupplier(SUPPLIERS.find(s => s.name === e.target.value)!)}
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5 text-[11px]">
            ...
            <SupStat label="Fulfillment" value={`${supplier.fulfill}%`} color="var(--brand-green)" />
          </div>

          <h2 className="mt-5 text-xs font-extrabold uppercase tracking-wider text-muted-foreground">Purchase Summary</h2>
          <div className="mt-2 rounded-2xl bg-[var(--brand-blue)] p-4 text-white">
            <div className="text-[11px] font-bold uppercase opacity-70">Estimated Procurement Value</div>
      ...
      <div className="flex items-center justify-end gap-2 border-t-2 bg-[var(--surface)] px-5 py-3 shadow-[0_-4px_0_var(--color-border)]">
        <div className="mr-auto text-xs font-bold text-muted-foreground">
          Budget Used: <span className="text-base font-extrabold tabular-nums text-[var(--brand-blue)]">{formatINR(totalValue)}</span> / {formatINR(budget)}
        </div>
...
function Kpi({ icon: Icon, color, label, value, sub }: { icon: typeof Wallet; color: string; label: string; value: string; sub: string }) {
  return (
    <div className="rounded-2xl border-2 bg-white p-3">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <div className="text-xl font-extrabold leading-none tabular-nums" style={{ color }}>{value}</div>
        </div>
      </div>
...
function SupStat({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="rounded-lg border-2 p-2">
      ...
      <div className="text-sm font-extrabold tabular-nums" style={{ color: color ?? "var(--ink)" }}>{value}</div>
    </div>
  );
}
